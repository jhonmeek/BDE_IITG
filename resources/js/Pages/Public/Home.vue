...
<template>
    <SiteLayout title="Accueil">
        ...
        <section class="hero-panel overflow-hidden rounded-2xl px-5 py-8 sm:px-10 sm:py-10 lg:px-12">
            <div class="grid items-center gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:gap-10">
                <div>
                    <p class="eyebrow-on-dark">Comité pour l’Épanouissement Académique</p>
                    <h1 class="page-title mt-3 text-white">
                        {{ settings.hero_title || 'Bureau des Étudiants IITG' }}
                    </h1>
                    <p class="lead mt-5 text-slate-300">
                ...
                <div class="legacy-photo-frame">
                    <img
                        ...
                        alt="Vie étudiante sur le campus IITG"
            ...
            <div class="mt-10 grid grid-cols-2 gap-px overflow-hidden rounded-xl bg-white/10 sm:grid-cols-4">
                <div class="bg-white/5 px-4 py-4 sm:px-5">
                ...
                <div class="bg-white/5 px-4 py-4 sm:px-5">
                    ...
                    <p class="mt-1 text-sm text-slate-300">Événements</p>
                </div>
                <div class="bg-white/5 px-4 py-4 sm:px-5">
        ...
        <section class="mt-14">
            <div class="flex flex-wrap items-end justify-between gap-4">
                <div>
                    ...
                    <h2 class="section-title section-title--rule mt-2">L’équipe du bureau</h2>
                    <p class="section-copy mt-4">Les membres qui portent la vie étudiante au quotidien.</p>
                </div>
                ...
                >
                    <div class="flex items-center gap-4">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-accent mt-0.5 text-sm font-medium">{{ member.role_title }}</p>
                        </div>
                    </div>
        ...
        <section class="mt-14 grid gap-8 lg:grid-cols-2">
            <div>
                <div class="flex flex-wrap items-end justify-between gap-4">
                    <h2 class="section-title section-title--rule">Nos clubs</h2>
                    <Link :href="route('clubs.index')" class="link-accent">
                        Tous les clubs →
                    </Link>
                </div>
                ...
                <div class="mt-6 space-y-4">
                    <article
                        v-for="club in clubs"
                        :key="club.id"
                        class="shell-card flex flex-col gap-4 p-4 sm:flex-row"
                    >
                        <img
                            v-if="club.image_url"
                            :src="club.image_url"
                        ...
                        <div class="min-w-0">
                            ...
                            <Link :href="route('clubs.show', club.slug)" class="link-accent mt-2">
                                Découvrir le club →
                            </Link>
                        </div>
                    </article>
            ...
            <div>
                <div class="flex flex-wrap items-end justify-between gap-4">
                    <h2 class="section-title section-title--rule">Prochains rendez-vous</h2>
                    <Link :href="route('events.index')" class="link-accent">
                        Agenda complet →
                    </Link>
                </div>
        ...
        <section class="mt-14 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
            <div class="shell-card card-pad">
                <h2 class="section-title section-title--rule">Le bureau au fil des mandats</h2>
                <div class="mt-6 space-y-8 border-l-2 pl-5 sm:pl-6" style="border-color: var(--bde-gold-soft);">
                    <article v-for="entry in history" :key="entry.id" class="relative">
                        <span class="absolute -left-[27px] top-1.5 h-3 w-3 rounded-full border-2 border-white sm:-left-[31px]" style="background: var(--bde-gold);" />
                        <p class="text-accent text-sm font-semibold">{{ entry.period_label }}</p>
            ...
            <div class="shell-card card-pad">
                <h2 class="section-title section-title--rule">En images</h2>
                <div class="mt-6 grid gap-4 sm:grid-cols-2">
                    <article v-for="item in media" :key="item.id">
                        <div class="aspect-video overflow-hidden rounded-lg bg-slate-100">
                            ...
                            <div v-else class="flex h-full items-center justify-center text-sm text-slate-500">Média</div>
                        </div>
                        <h3 class="mt-3 text-sm font-semibold">{{ item.title }}</h3>
                        <p class="mt-1 text-sm text-slate-500">{{ item.caption }}</p>
                    </article>
                </div>
                <Link :href="route('media.index')" class="btn-secondary mt-6">Voir tous les médias</Link>
            </div>
        </section>
    </SiteLayout>
</template>
